...
# Appendix A: Debugging Hermes TUI Slash Commands

Hermes slash commands span three layers: Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync.
...
TUI gateway (tui_gateway/server.py)         <- slash.exec / command.dispatch
       │
       ▼
Ink frontend (tui_gateway/src/)             <- slash command UI + autocomplete
```

**Diagnostic steps:**
1. Check `COMMAND_REGISTRY` in `hermes_cli/commands.py` — is the command registered?
2. Check `tui_gateway/server.py` — does the gateway expose it via `slash.exec`?
3. Check `tui_gateway/src/` — is the TypeScript frontend mapping it?
4. Verify consistency across all three layers.

---

# Appendix B: Skill Authoring (In-Repo)

Write skills that ship with Hermes itself under `/home/bb/hermes-agent/skills/<category>/<name>/SKILL.md`. Use `write_file` + `git add`; `skill_manage(action='create')` does NOT target this tree.
...
- Optional: `version`, `author`, `license`, `platforms`, `dependencies`, `prerequisites`.

**Validation:** run `python tools/skill_manager_tool.py --validate skills/<category>/<name>/SKILL.md`

**In-repo vs user-local:**
- In-repo: committed, shipped with the package, lives under `skills/<category>/<name>/`
- User-local: personal, created via `skill_manage(action='create')`, lives under `~/.hermes/skills/`

---

# Appendix C: s6-overlay Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the Hermes Agent Docker image.
...
- Understanding why the container's CMD is `/opt/hermes/docker/main-wrapper.sh`
...
├── s6-rc.d
│   ├── hermes-dashboard (static service)
│   └── hermes-gateway-<profile> (per-profile)
└── user program (main-wrapper.sh)
```

**Key files:**
- `docker/main-wrapper.sh` — container entry point
- `docker/stage2-hook.sh` — cont-init.d boot script
- `docker/s6-services/` — static service definitions
- `docker/render-services.py` — renders per-profile gateway run scripts
